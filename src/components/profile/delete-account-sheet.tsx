...
/** Keep in sync with post-admin-sheet / create-post-sheet. */
const SHEET_TRANSITION_MS = 320
const ADMIN_MENU_ICON_STROKE = 2 as const

/** Same destructive surface as מחק את הפוסט in {@link PostAdminSheet} delete confirm. */
const DELETE_ACCOUNT_BTN_ROW = cn(
...
type Props = {
  ...
  busy: boolean
  error: string | null
  /** Called after user confirms — should await server delete then sign-out + navigate. */
...
/** Bottom sheet — mirrors post-delete confirm (מחיקת פוסט) for account deletion. */
export function DeleteAccountSheet({
  ...
  return createPortal(
    <div
      data-loby-delete-account-sheet=""
        ...
        <div className="flex min-w-0 w-full flex-col pb-[calc(1rem+env(safe-area-inset-bottom,0px))] pt-[max(env(safe-area-inset-top,0px),0.75rem)] text-sm text-popover-foreground">
          ...
          <div className="mt-10 flex flex-col gap-5 px-4 pb-5 pt-0">
            ...
            <p className="text-start text-[0.9375rem] leading-relaxed text-foreground">
              לאחר אישור, החשבון יימחק לצמיתות ולא ניתן יהיה לשחזר אותו. לא תוכלו להתחבר שוב באותם פרטים
              ותצטרכו להירשם מחדש דרך האפליקציה גם אם תרצו להשתמש שוב באותה כתובת {`דוא"ל`}. האם למחוק?
            </p>
